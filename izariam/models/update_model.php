               ...
               for ($is = 0; $is <= 1; $is++)
               {
                   ...
                   if ($island->$res_start > 0)
                   {
                       ...
                       if ($elapsed_wood >= $cost['time'])
                       {
                           ...
                           $text = '<b>'.$res_name.'</b> has extended on island <a href="'.$this->config->item('base_url').'game/island/'.$island->id.'/">'.$island->name.' ('.$island->x.':'.$island->y.')</a>!';
                           for ($i = 0; $i <= 15; $i++)
                           {
                               $town_text = 'city'.$i;
                   ...
                   if ($mission->return_start == 0)
                   {
                       ...
                       if ($mission->loading_to_start > 0 and $loading_end <= 0)
                       {
                           ...
                           elseif($mission->mission_type == 2)
                           {
                                   ...
                                   if ($mission->sulfur > 0){$text .= '<li class="sulfur"><span class="textLabel">Sulfur: </span>'.($mission->sulfur).'</li>';}
                                   $text .= '</ul>';
                                   $text_from = 'Your merchant marine fleet from <a href="'.$this->config->item('base_url').'game/island/'.$this->Data_Model->temp_towns_db[$mission->from]->island.'/'.$mission->from.'/">'.$this->CI->Data_Model->temp_towns_db[$mission->from]->name.'</a> has arrived <a href="'.$this->config->item('base_url').'game/island/'.$this->Data_Model->temp_towns_db[$mission->to]->island.'/'.$mission->to.'/">'.$this->CI->Data_Model->temp_towns_db[$mission->to]->name.'</a> and brought the following goods: <ul class="resources">'.$text;
                                   if ($mission->user != $trade_town->user)
                                   {        
                           ...
                           elseif($mission->mission_type == 4)
                           {
                               ...
                               {
                                   $text = '';
                                   if ($wood > 0){ $text .= '<li class="wood"><span class="textLabel">Building materials: </span>'.($wood).'</li> Cost:<li class="gold"><span class="textLabel">Gold: </span>'.$trade_town->branch_trade_wood_cost.'</li><br>';}
                                   if ($wine > 0){$text .= '<li class="wine"><span class="textLabel">Grapes: </span>'.($wine).'</li> Cost:<li class="gold"><span class="textLabel">Gold: </span>'.$trade_town->branch_trade_wine_cost.'</li><br>';}
                                   if ($crystal > 0){$text .= '<li class="marble"><span class="textLabel">Marble: </span>'.($marble).'</li> Cost:<li class="gold"><span class="textLabel">Gold: </span>'.$trade_town->branch_trade_marble_cost.'</li><br>';}
                                   if ($marble > 0){$text .= '<li class="glass"><span class="textLabel">Crystal: </span>'.($crystal).'</li> Cost:<li class="gold"><span class="textLabel">Gold: </span>'.$trade_town->branch_trade_crystal_cost.'</li><br>';}
                                   if ($sulfur > 0){$text .= '<li class="sulfur"><span class="textLabel">Sulfur: </span>'.($sulfur).'</li> Cost:<li class="gold"><span class="textLabel">Gold: </span>'.$trade_town->branch_trade_sulfur_cost.'</li><br>';}
                                   $text .= '</ul>';
                                   // Сообщение отправителю
