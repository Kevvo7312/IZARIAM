                   ...
                   while (SizeOf($buildings) > 0)
                   {
                       ...
                       if (($this->CI->$model->towns[$i]->build_start + $cost['time']) <= time())
                       {
                           if (($step == 0) or ($step > 0 and $wood >= 0 and $marble >= 0 and $wine >= 0 and $crystal >= 0 and $sulfur >= 0))
                           {
                                    ...
                                    $message = ($this->CI->$model->towns[$i]->$level_text == 1) ? 'Your building "<a href="'.$this->config->item('base_url').'game/city/'.$i.'/'.$this->Data_Model->building_class_by_type($buildings[0]['type']).'/'.$buildings[0]['position'].'/">'.$this->Data_Model->building_name_by_type($buildings[0]['type']).'</a>" has been completed!' : 'Your building "<a href="'.$this->config->item('base_url').'game/city/'.$i.'/'.$this->Data_Model->building_class_by_type($buildings[0]['type']).'/'.$buildings[0]['position'].'/">'.$this->Data_Model->building_name_by_type($buildings[0]['type']).'</a>" was expanded to level '.$this->CI->$model->towns[$i]->$level_text.'!';
                                    $town_message = array(
               ...
               for ($is = 0; $is <= 1; $is++)
               {
                   ...
                   if ($island->$res_start > 0)
                   {
                       ...
                       if ($elapsed_wood >= $cost['time'])
                       {
                           ...
                           {
                               case 1: $word = 'has extended'; break;
                               case 2: $word = 'расширился'; break;
                               default: $word = 'has extended'; break;
                           }
                   ...
                   if ($mission->return_start == 0)
                   {
                       if ($mission->loading_to_start == 0)
                       {
                           ...
                           elseif($mission->mission_type == 3 and $mission_end <= 0)
                           {
                               ...
                               if ($gold > 0)
                               {
                                   ...
                                   $text = '';
                                   if ($mission->trade_wood_count > 0){ $text .= '<li class="wood"><span class="textLabel">Building materials: </span>'.($mission->trade_wood_count).'</li> Cost:<li class="gold"><span class="textLabel">Gold: </span>'.$trade_town->branch_trade_wood_cost.'</li><br>';}
                                   if ($mission->trade_wine_count > 0){$text .= '<li class="wine"><span class="textLabel">Grapes: </span>'.($mission->trade_wine_count).'</li> Cost:<li class="gold"><span class="textLabel">Gold: </span>'.$trade_town->branch_trade_wine_cost.'</li><br>';}
                                   if ($mission->trade_marble_count > 0){$text .= '<li class="marble"><span class="textLabel">Marble: </span>'.($mission->trade_marble_count).'</li> Cost:<li class="gold"><span class="textLabel">Gold: </span>'.$trade_town->branch_trade_marble_cost.'</li><br>';}
                                   if ($mission->trade_crystal_count > 0){$text .= '<li class="glass"><span class="textLabel">Crystal: </span>'.($mission->trade_crystal_count).'</li> Cost:<li class="gold"><span class="textLabel">Gold: </span>'.$trade_town->branch_trade_crystal_cost.'</li><br>';}
                                   if ($mission->trade_sulfur_count > 0){$text .= '<li class="sulfur"><span class="textLabel">Sulfur: </span>'.($mission->trade_sulfur_count).'</li> Cost:<li class="gold"><span class="textLabel">Gold: </span>'.$trade_town->branch_trade_sulfur_cost.'</li><br>';}
                                   $text .= '</ul>';
                                   // Сообщение отправителю
                                   $text_from = 'Your merchant marine fleet from <a href="'.$this->config->item('base_url').'game/island/'.$this->Data_Model->temp_towns_db[$mission->from]->island.'/'.$mission->from.'/">'.$this->CI->Data_Model->temp_towns_db[$mission->from]->name.'</a> has arrived in <a href="'.$this->config->item('base_url').'game/island/'.$this->Data_Model->temp_towns_db[$mission->to]->island.'/'.$mission->to.'/">'.$this->CI->Data_Model->temp_towns_db[$mission->to]->name.'</a> and now docking: <ul class="resources">'.$text;
                                   ...
                                   if ($mission->user != $trade_town->user)
                                   {
                                       ...
                                       $text_to = 'Merchant marine fleet from <a href="'.$this->config->item('base_url').'game/island/'.$this->Data_Model->temp_towns_db[$mission->from]->island.'/'.$mission->from.'/">'.$this->CI->Data_Model->temp_towns_db[$mission->from]->name.'</a> has arrived in your port <a href="'.$this->config->item('base_url').'game/island/'.$this->Data_Model->temp_towns_db[$mission->to]->island.'/'.$mission->to.'/">'.$this->CI->Data_Model->temp_towns_db[$mission->to]->name.'</a> and now docking: <ul class="resources">'.$text;
                       ...
                       if ($mission->loading_to_start > 0 and $loading_end <= 0)
                       {
                           if($mission->mission_type == 1)
                           {
                               ...
                               // Сообщение
                               $text = 'We have based new city (<a href="'.$this->config->item('base_url').'game/island/'.$this->Data_Model->temp_towns_db[$mission->to]->island.'/'.$mission->to.'/">'.$this->CI->Data_Model->temp_towns_db[$mission->to]->name.'</a>). Your merchant marine fleet has unloaded: <ul class="resources">';
                               $text .= '<li class="wood"><span class="textLabel">Building materials: </span>'.($mission->wood-1000).'</li>';
                               if ($mission->wine > 0){$text .= '<li class="wine"><span class="textLabel">Grapes: </span>'.($mission->wine).'</li>';}
                           ...
                           elseif($mission->mission_type == 2)
                           {
                                   ...
                                   if ($mission->sulfur > 0){$text .= '<li class="sulfur"><span class="textLabel">Sulfur: </span>'.($mission->sulfur).'</li>';}
                                   $text .= '</ul>';
                                   $text_from = 'Your merchant marine fleet from <a href="'.$this->config->item('base_url').'game/island/'.$this->Data_Model->temp_towns_db[$mission->from]->island.'/'.$mission->from.'/">'.$this->CI->Data_Model->temp_towns_db[$mission->from]->name.'</a> has arrived в <a href="'.$this->config->item('base_url').'game/island/'.$this->Data_Model->temp_towns_db[$mission->to]->island.'/'.$mission->to.'/">'.$this->CI->Data_Model->temp_towns_db[$mission->to]->name.'</a> has brought the following goods: <ul class="resources">'.$text;
                                   if ($mission->user != $trade_town->user)
                                   {        
                                       $text_to = 'Merchant marine fleet from <a href="'.$this->config->item('base_url').'game/island/'.$this->Data_Model->temp_towns_db[$mission->from]->island.'/'.$mission->from.'/">'.$this->CI->Data_Model->temp_towns_db[$mission->from]->name.'</a> has arrived in your city <a href="'.$this->config->item('base_url').'game/island/'.$this->Data_Model->temp_towns_db[$mission->to]->island.'/'.$mission->to.'/">'.$this->CI->Data_Model->temp_towns_db[$mission->to]->name.'</a> has brought the following goods: <ul class="resources">'.$text;
                                       $town_to_message = array(
                                                'user' => $trade_town->user,
                                                'town' => $mission->to,
                                                'date' => $mission->mission_start + $time,
                           ...
                           elseif($mission->mission_type == 4)
                           {
                               ...
                               {
                                   $text = '';
                                   if ($wood > 0){ $text .= '<li class="wood"><span class="textLabel">Building materials: </span>'.($wood).'</li> по цене:<li class="gold"><span class="textLabel">Gold: </span>'.$trade_town->branch_trade_wood_cost.'</li><br>';}
                                   if ($wine > 0){$text .= '<li class="wine"><span class="textLabel">Grapes: </span>'.($wine).'</li> по цене:<li class="gold"><span class="textLabel">Gold: </span>'.$trade_town->branch_trade_wine_cost.'</li><br>';}
                                   if ($crystal > 0){$text .= '<li class="marble"><span class="textLabel">Marble: </span>'.($marble).'</li> по цене:<li class="gold"><span class="textLabel">Gold: </span>'.$trade_town->branch_trade_marble_cost.'</li><br>';}
                                   if ($marble > 0){$text .= '<li class="glass"><span class="textLabel">Crystal: </span>'.($crystal).'</li> по цене:<li class="gold"><span class="textLabel">Gold: </span>'.$trade_town->branch_trade_crystal_cost.'</li><br>';}
                                   if ($sulfur > 0){$text .= '<li class="sulfur"><span class="textLabel">Sulfur: </span>'.($sulfur).'</li> по цене:<li class="gold"><span class="textLabel">Gold: </span>'.$trade_town->branch_trade_sulfur_cost.'</li><br>';}
                                   $text .= '</ul>';
                                   // Сообщение отправителю
                                   $text_from = 'Your merchant marine fleet from <a href="'.$this->config->item('base_url').'game/island/'.$this->Data_Model->temp_towns_db[$mission->from]->island.'/'.$mission->from.'/">'.$this->CI->Data_Model->temp_towns_db[$mission->from]->name.'</a> has arrived in <a href="'.$this->config->item('base_url').'game/island/'.$this->Data_Model->temp_towns_db[$mission->to]->island.'/'.$mission->to.'/">'.$this->CI->Data_Model->temp_towns_db[$mission->to]->name.'</a> has also unloaded: <ul class="resources">'.$text;
                                   $town_from_message = array(
                                       'user' => $mission->user,
                                       'town' => $mission->from,
                                       'date' => $mission->mission_start + $time,
                   ...
                   else
                   {
                       if ($return_end <= 0 or ($loading_end == 0 and $this->CI->Data_Model->temp_towns_db[$mission->to]->user == $mission->user))
                       {
                           ...
                           if ($mission->wood > 0 or $mission->wine > 0 or $mission->marble > 0 or $mission->crystal > 0 or $mission->sulfur > 0 or $mission->peoples > 0 or $mission->gold > 0)
                           {
                               ...
                               // Отправляем сообщение
                               $text = 'Your merchant marine fleet from <a href="'.$this->config->item('base_url').'game/island/'.$this->Data_Model->temp_towns_db[$mission->from]->island.'/'.$mission->from.'/">'.$this->CI->Data_Model->temp_towns_db[$mission->from]->name.'</a> has returned;
                               if ($mission->gold > 0 or $mission->wood > 0 or $mission->wine > 0 or $mission->marble > 0 or $mission->crystal > 0 or $mission->sulfur > 0)
                               {
                                   ...
                                   if ($mission->gold > 0){ $text .= '<li class="gold"><span class="textLabel">Gold: </span>'.($mission->gold).'</li>';}
                                   if ($mission->wood > 0){ $text .= '<li class="wood"><span class="textLabel">Building materials: </span>'.($mission->wood).'</li>';}
                                   if ($mission->wine > 0){$text .= '<li class="wine"><span class="textLabel">Grapes: </span>'.($mission->wine).'</li>';}
                                   if ($mission->marble > 0){$text .= '<li class="marble"><span class="textLabel">Marble: </span>'.($mission->marble).'</li>';}
                                   if ($mission->crystal > 0){$text .= '<li class="glass"><span class="textLabel">Crystal: </span>'.($mission->crystal).'</li>';}
