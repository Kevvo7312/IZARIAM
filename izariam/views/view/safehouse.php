...
<div class="yui-navset">
    <ul class="yui-nav">
        <li <?if($param2!='reports' and $param2!='archive'){?>class="selected"<?}?>><a href="<?=$this->config->item('base_url')?>game/safehouse/<?=$position?>/" title="Hideout"><em>Hideout</em></a></li>
        <li <?if($param2=='reports'){?>class="selected"<?}?>><a href="<?=$this->config->item('base_url')?>game/safehouse/reports/" title="Reports"><em>Reports</em></a></li>
...
<?if($param2!='reports' and $param2!='archive'){?>
    <div class="contentBox01h">
        <h3 class="header"><span class="textLabel">Training</span></h3>
        <div class="content">
            <ul id="units">
                <li class="unit">
                    <div class="unitinfo">
                        ...
                        <p>This loyal citizen is the ideal candidate for a spy, recruit now to begin training.</p>
                    </div>
                    <div class="forminput">
...
<?if(($this->Player_Model->levels[$this->Player_Model->town_id][14]-$all_spyes) > 0){?>
                        <div class="centerButton">
                            <a href="<?=$this->config->item('base_url')?>actions/spyes/buy/" class="button" title="Training">Recruit!</a>
                        </div>
<?}else{?>
                        Maximum spies reached!
<?}}?>
<?}else{?>
                        Training
<?}?>
...
<div class="contentBox01h">
  <h3 class="header"><span class="textLabel">Missions</span></h3>
  <div class="content">
...
<?if($spy->mission_start > 0 and $spy->mission_type != 2){?>
              <li title="Status" class="status">spy on mission</li>
<?}?>
...
?>
              <li class="risk"><span class="textLabel">Risk of exposure</span>:<br>
                  <div class="statusBar">
...
<?if($spy->mission_start == 0){?>
          <div class="missionButton">
              <a title="Task Spy" href="<?=$this->config->item('base_url')?>game/safehouseMissions/<?=$spy->id?>/">Mission</a>
          </div>
          <div class="missionAbort">
              <a title="Withdraw from mission" href="<?=$this->config->item('base_url')?>actions/spyes/return/<?=$spy->id?>/<?=$spy->from?>/">Response</a>
          </div>
<?}?>
      </div>
<?}?>
<?}else{?>
        <center><div style="padding:10px;">There are no spies on missions!</div></center>
<?}?>
      </div>
  <div class="footer"></div>
</div>
...
<?foreach($this->Player_Model->spyes_messages as $messages){?>
                ...
                <tr><th colspan="3">Spy report from <?=$this->Data_Model->temp_towns_db[$messages[0]->to]->name?></th></tr>
...
<div id="troopsOverview">
        <div class="contentBox01h">
            <h3 class="header"><span class="textLabel">Archived spy reports</span></h3>
            <div class="content">
                <p style="text-align: center;"><i>There are no spy reports in the archive.</i></p>
            </div>
            <div class="footer"></div>
        </div>
    </div>
